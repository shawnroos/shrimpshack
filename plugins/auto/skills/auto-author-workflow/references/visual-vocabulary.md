# Visual vocabulary — the workflow topology card

This is the contract doc for `lib/topology-render.py::render(workflow, width)` — the
...
`render` derives the card from workflow STRUCTURE, so a user-authored workflow looks
like a built-in:
...
Render each with `bash "${CLAUDE_PLUGIN_ROOT}/lib/workflows-list.sh" --render <name>`:
...
`lib/topology-render.py::render_comparison(workflows, *, highlight=None, width=60)`
...
holds: a comparison is just N invocations of `render`, and a user-authored workflow
...
- The card whose workflow `name` equals `highlight` is prefixed with a
  `► recommended` marker line; `highlight=None` (or a name not among the
  candidates) emits no marker.
- A single-workflow list renders one card with no separator artifacts. Output is
  deterministic (byte-identical across calls), so tests can assert it exactly.

Render a contrast block with
`bash "${CLAUDE_PLUGIN_ROOT}/lib/workflows-list.sh" --compare <name>... [--highlight <name>]`
— it resolves each candidate through the same first-wins `workflows.resolve` as
