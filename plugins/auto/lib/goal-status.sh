...
#   feed it the run-record result. So auto ships its OWN thin Stop hook
...
#   that could drift from the run-record predicate — there is therefore NO staleness
#   window where this status says done while the run-record says not (e.g. when a
#   re-review flips verdict-returned → pending and reopens the predicate). Every
#   invocation re-reads the run-record; `done` IS `exit_predicate_result.met` from
...
#     iterations  — best-effort advance count (placeholder 0; the run-record has no
...
#   Prints the deterministic status JSON for ONE run. Reads the run-record
#   lock-free (atomic-rename invariant => consistent snapshot; well under any
#   hook timeout). On a missing/malformed run-record, emits active:false (allow
