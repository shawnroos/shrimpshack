...
# the `lib/workflows.py` validate/resolve write-gate (the run-scoped variant). A
...
# workspace workflow; two_step+gated ⇒ the a2/a4 variant compiles+validates+
...
#
# SELF-CONTAINED inline harness (same style as launch-workflow-compile.test.sh):
...
# operator's real workflows; the workspace tier lives under a mktemp repo so any
# teardown `rm` stays inside ephemeral $TMPDIR.
#
# Scenarios (mapped to the U7 plan's Test scenarios):
#   AE1  typo-fix → skip; router agrees on a1; a1 no-compile (no workspace workflow);
...
# Prints "<workflow_or_entry> <entry> <kind>" — the deterministic agreement
# cross-check classify_launch's `router_agrees` is derived from.
router() {
  "$PY" "${AUTO_ROOT}/lib/recommender.py" "$1" \
    | "$PY" -c 'import sys,json; r=json.load(sys.stdin); print(r["workflow_or_entry"], r["entry"], r["kind"])'
}
...
def atomic_write(path, workflow):
    """mkstemp + os.rename — the auto-author-workflow write discipline."""
...
def compile_variant(builtin, slug):
    ...
    rb, _ = workflows.load_and_validate(draft["name"], repo)   # raises on bad read-back
    _, vtier = workflows.resolve(draft["name"], repo)
    _, btier = workflows.resolve(builtin, repo)
...
if op == "ae3-compile-a2":
    ...
    idx = lines.index(MARK) if MARK in lines else -1
    after = lines[idx + 1] if 0 <= idx and idx + 1 < len(lines) else ""
    has_custom_header = ("workflow: " + custom["name"]) in out
    ...
    # the skip path writes nothing to .claude/auto/workflows.
...
assert_eq "a1 plan workflow" "$A1_ROUTER"
...
assert_eq "w work workflow" "$(router reviewed-plan)"
...
assert_eq "markers=1|after=workflow: a2" "$(drv ae3-render)"
...
# ════════════════════════════════════════════════════════════════════════════
it "AE4: custom workflow (0.9,0.9,custom,[],True) → two_step (rule 1, R4 — always drawn)"
...
assert_eq "valid=valid|markers=1|after=workflow: spike-spike|customhdr=True" "$AE4"

# ════════════════════════════════════════════════════════════════════════════
# AE5 — the operator overrides the recommended a2 and picks a4 at step 1 → the
#   compiled run-scoped workflow is a4-<slug>, gate_step `compare` (NOT a2's
...
#   shape is distinct from a2's by the gate_step the compiled workflow carries.)
# ════════════════════════════════════════════════════════════════════════════
...
assert_eq "a1 plan workflow" "$(router clear-intent-no-plan)"
...
it "skip dispatch shape: AE1/AE2 a1/w never wrote a workspace workflow (the gated AEs did)"
...
WS_LEAK="$(ls "${WORKSPACE_WORKFLOWS}" 2>/dev/null | grep -cE '^(a1|w)-' || true)"
assert_eq "0" "$WS_LEAK"
