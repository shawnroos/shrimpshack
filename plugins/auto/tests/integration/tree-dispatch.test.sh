...
# performs ONLY the `pending -> dispatched` run-record transition. `lib/dispatcher.py`
# is unchanged by U5.
#
...
#      sub-agent, via `python3 lib/run_record.py record-verdict`) is converged by a
...
# rely on separate `python3` PROCESSES sharing the same run-record through
...
# work-loop op). Runs in one python process; the run-record persists on disk.
seed_run() {
...
def load(name):
    ...
    m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)
    return m
orch = load("dispatcher"); run_record = load("run_record")
repo = os.environ["CLAUDE_AUTO_REPO"]
before = run_record.read_run_record(repo, run)["steps"][0].get("attempt", 0)
orch.dispatch_batch(repo, run, ["u1"], cap=1)
after = run_record.read_run_record(repo, run)["steps"][0].get("attempt", 0)
...
# from sub-agent return text instead of the run-record, this would be impossible.
seed_run C 1
...
"$PY" "$AUTO_ROOT/lib/run_record.py" record-verdict C u1 '[]' 1 >/dev/null 2>&1
vrc=$?
...
it "stale verdict (attempt 0 < current 1) is REJECTED (StaleVerdict) — non-zero exit, run_record unchanged"
"$PY" "$AUTO_ROOT/lib/run_record.py" record-verdict D u1 '[]' 0 >/dev/null 2>&1
stale_rc=$?
state_after="$("$PY" "$AUTO_ROOT/lib/run_record.py" read "$CLAUDE_AUTO_REPO" D 2>/dev/null | "$PY" -c 'import sys,json; d=json.load(sys.stdin); print(d["steps"][0]["state"])')"
...
"$PY" "$AUTO_ROOT/lib/run_record.py" record-verdict D u1 '[]' 1 >/dev/null 2>&1
fresh_rc=$?
state_fresh="$("$PY" "$AUTO_ROOT/lib/run_record.py" read "$CLAUDE_AUTO_REPO" D 2>/dev/null | "$PY" -c 'import sys,json; d=json.load(sys.stdin); print(d["steps"][0]["state"])')"
...
base = run_record.parse_iso(dispatched_at)
...
state_alive = run_record.read_run_record(repo, run)["steps"][0]["state"]

# PAST THRESHOLD: 3601s in (age > 3600) -> stalled.
led2 = run_record.read_run_record(repo, run)
dead = base + timedelta(seconds=3601)
_, halted_dead, confirmed_dead = ta.detect_and_halt_stalled(repo, run, led2, dead)
state_dead = run_record.read_run_record(repo, run)["steps"][0]["state"]
