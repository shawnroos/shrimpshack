...
#   - lib/iteration.py — the sole READER of the iteration-decision field; this
#     module is the centralized decision point every caller routes through.
#   - lib/ledger.py    — the sole WRITER of the iteration-decision field (via
#                        set_verdict_decision); also stores the field name in
#                        schema constants. Reads in ledger.py route through
#                        iteration.read_decision().
#   - lib/on-stop.py   — uses the literal "decision" for the CLAUDE CODE HOOK
#                        PROTOCOL ({"decision":"block"} in stop-hook output) —
#                        a different "decision" than v0.3.0's iteration decision.
#                        Pre-dates v0.3.0; harness contract, not internal data.
#
# NOTE: ledger.py's allowance is narrow — it may CONSTRUCT/WRITE the field, but
# any decision-comparison logic in ledger.py (e.g. recompute_predicate's
# iteration_pending check) must use iteration.read_decision(). on-stop.py's
# allowance covers the harness-protocol literal specifically. This test asserts
# the literal's ABSENCE outside the three allowed files; a finer check
# (write-only vs compare; hook-protocol vs iteration-protocol) lives in code review.
...
ALLOWED = {"iteration.py", "ledger.py", "on-stop.py"}
LITERAL = "decision"
...
it "no 'decision' string literal outside iteration.py / ledger.py"
result="$(run_lint)"
