...
team_view_row() {       # <index> <member json>
    ...
    mfields="$(printf '%s' "$m" | jq -r '(.name // ""), (.launch_state // ""),
        ...
        (.failure | tojson), (.served_model | tojson), (.grants | tojson)')"
    local has_outcome fail_json sm_json gr_json
    {
        read -r name
        read -r ls
        read -r handle
...
EOF
    ...
    jq -nc --arg n "$name" --arg a "$alias" --arg w "$wt" --arg ls "$ls" \
        ...
        --argjson gr "${gr_json:-null}" '{
          ...
          failure:$fail, served_model:$sm, grants:$gr,
