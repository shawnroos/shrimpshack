...
do_describe() {
    ...
    emit "$(jq -nc "$(spawn::envelope_jq plugin)"' + {
      ...
      error_values:[
        ...
        {value:"contract_invalid",      exit_code:null, note:"a MEMBER-row value only, at members[].error — never this surface’s top-level `error`, so branching on it beside a process exit code matches nothing. It rides a `launch_failed` response at exit 5 here, while bg-agent’s own --describe declares it at exit 2. Cause: that member’s contract is not one JSON object with a non-empty `task` and a non-empty `deliverables` array. bg-agent reads the contract after dispatch, so the round opens and the checkout is placed before this is known; `teardown` removes that checkout when you run it"},
