...
remedy_for() {
    case "$1" in
        ...
        skill_not_provisioned)
            printf 'This member’s contract instructs a slash command it was never provisioned the skill for, and it was refused before its launcher started anything. Do NOT retry it: the contract and the member’s `skills` array are both fixed on the record, so the same refusal lands every time. Read `detail` for the skill the contract named — then either add it to that member’s `skills` in the team file, or reword the contract when the slash command was a mention rather than an instruction — and start a new run.' ;;
        skill_unresolvable)
            printf 'This member names a skill that does not resolve to anything installed, so it was refused before its launcher started anything rather than left to run without the method it was promised. Do NOT retry it: the `skills` array on the record does not change between attempts. Read `detail` for the name — it resolves from your own skills and from installed plugins, in bare or `plugin:skill` form — fix it in the team file, and start a new run.' ;;
        driver_worktree)
...
do_describe() {
    ...
    emit "$(jq -nc "$(spawn::envelope_jq plugin)"' + {
      ...
      response_fields:[
        ...
        {name:"members[].skills_landed", always:false, note:"the skills that were actually PROVISIONED to this member, read from the child’s own result once it reaches one. null means NO RESULT YET, never `none`. Never derived from members[].skills, which is what the team file ASKED for: a skill that did not land must never read as one that did, the same separation allow and grants keep. A member refused before its launcher ran — including one refused because its contract instructs a skill it was not given — keeps null here for ever"},
      ...
      team_file_fields:[
        ...
        {name:"members", required:true,  note:"one entry per member, dispatched in the order written",
         member_fields:[
           ...
           {name:"contract", required:true,  note:"path to that member’s own contract file. The file is ONE JSON object with a non-empty `task` and a non-empty `deliverables` array of worktree-relative paths; `done_means` is optional prose; `verify` is an optional SHELL COMMAND the supervisor eval-runs in that member’s worktree after the child finishes, and its exit code grades the result — a sentence written there is executed and fails. This surface hands the file to bg-agent unread and never parses it, so a contract written as markdown or prose dispatches from here and then fails that member with `contract_invalid` after its worktree is already placed. bg-agent.sh --describe carries the field-by-field contract"},
      ...
      error_values:[
        ...
        {value:"launch_failed",         exit_code:5, note:"a member’s launcher refused it; that member carries the launcher’s own error value. Read bg-agent.sh --describe error_values for the full set; this table declares only THIS surface’s own top-level errors, and a member’s value is never one of them. The common one is `contract_invalid`: that member’s contract is not one JSON object with a non-empty `task` and a non-empty `deliverables` array. bg-agent reads it only after dispatch, so the round opens and the checkout is placed first; the caller’s own `teardown` removes that checkout"},
        {value:"skill_not_provisioned", exit_code:2, note:"retry was given a member whose contract instructs a slash command it was never provisioned the skill for. The contract and the member’s skills array are both immutable on the record, so the launcher would refuse identically on every attempt; refused here rather than spent against max_rounds. Fix the team file — add the skill to that member, or reword the contract — and dispatch a new run"},
        {value:"skill_unresolvable",    exit_code:2, note:"retry was given a member naming a skill that does not resolve. The member’s skills array is immutable on the record, so the name would fail to resolve identically on every attempt; refused here rather than spent against max_rounds. Fix the name in the team file and dispatch a new run"},
