...
Two things to check while you are there. The first now refuses the member; the second is silent:

- **A method the member cannot run refuses that member**, before anything is claimed, and the rest of the round still dispatches. Two different causes, two different fixes: `skill_not_provisioned` means the contract names a method as a literal `/name` and no entry in that member's `skills` matches it — add the skill, or reword the contract. `skill_unresolvable` means a name in `skills` resolves to nothing installed — fix the name. A name that resolves and then fails to copy is the case that still runs: it lands in that member's `failure.degraded_reasons[]`.
