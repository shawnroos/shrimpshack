...
- **A skill name that does not resolve refuses that member.** The launcher settles it `skill_not_provisioned` or `skill_unresolvable` before anything is claimed, and the rest of the round still dispatches — a typo costs you one member, not a member reporting as though it had a method it never got. A name that resolves and then fails to copy is the case that still runs: it lands in that member's `failure.degraded_reasons[]`.
