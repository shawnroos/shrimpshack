...
setup() {
    ...
    unset SPAWN_LOG SPAWN_LENS_TIMEOUT SPAWN_SPILL_BYTES
...
@test "sanitizer strips ESC, BEL, CR and DEL while keeping tab, newline and prose" {
    ...
    run bash -c '. "$1"; spawn::sanitize_for_display "$2"' _ "$LIB/sanitize.sh" "$POISON"
    [ "$status" -eq 0 ]
    ...
    run bash -c '. "$1"; spawn::sanitize_for_display "$2" | od -c | tr -d "\n"' _ \
        "$LIB/sanitize.sh" $'a\tb\nc'
    [ "$status" -eq 0 ]
    echo "$output" | grep -q '\\t'
    echo "$output" | grep -q '\\n'
...
@test "sanitizer strips Unicode bidi overrides and zero-width characters" {
    ...
    run bash -c '. "$1"; spawn::sanitize_for_display "$2"' _ "$LIB/sanitize.sh" "$PROSE"
    [ "$output" = "$PROSE" ]
}
...
@test "sanitizer stream form strips per line and preserves line structure" {
    ...
    bash -c '. "$1"; spawn::sanitize_stream < "$2" > "$3"' _ "$LIB/sanitize.sh" "$WORK/in.bin" "$WORK/got.bin"
    refute_bytes "$ESC" "$WORK/got.bin"
    refute_bytes "$BIDI" "$WORK/got.bin"
...
@test "KTD5 grammar: spawnctl ensure refuses an escape-laden alias, code 2, output clean" {
    BAD="al${ESC}[31mias"
...
@test "KTD5 grammar: a session id that fails the grammar is refused, no handle is printed" {
    ...
    export SPAWN_CLAUDE_BIN="$FIX/fake-claude.sh"
...
@test "served alias list: escape bytes off the wire are stripped from status output" {
    ...
    printf '%s' "$SPAWN_BASE_URL" > /dev/null
    curl -s -H "x-api-key: $TOKEN" "$SPAWN_BASE_URL/v1/models" > "$WORK/models.raw"
...
@test "gateway log tail: a poisoned log line is sanitized before it reaches the terminal" {
    ...
    export SPAWN_LOG="$WORK/gateway.log"
    printf 'boot line one\n%s\n' "$POISON" > "$SPAWN_LOG"
    assert_bytes "$ESC" "$SPAWN_LOG"
    assert_bytes "$BIDI" "$SPAWN_LOG"
...
EOF
    ...
    export SPAWN_CLAUDE_BIN="$WORK/bad-claude.sh"
...
terminal_sink_lint() {
    ...
    for f in "$dir"/*.sh; do
        ...
        while IFS= read -r line; do
            ...
            function defended(s) { return s ~ /spawn::sanitize_(for_display|stream)/ }
            { lines[NR] = $0 }
            END {
                sp = 0; nblk = 0
                for (i = 1; i <= NR; i++) {
