...
@test "KTD2: spawnctl emits one JSON object when the temp dir cannot be created" {
...
# spawnctl SUBPROCESS first. launch's own tmpwork emit_error is defensive
...
# mutation:
#   * revert spawnctl's tmpwork emit alone -> still green (the rewrap emits
...
@test "stop refuses ok:true when the recorded pid is dead but a gateway still serves" {
    ...
    mkdir -p "$SPAWN_SEARCH_ROOT/gateway-9.9.9/target/release"
    : > "$SPAWN_SEARCH_ROOT/gateway-9.9.9/target/release/gateway"
    chmod +x "$SPAWN_SEARCH_ROOT/gateway-9.9.9/target/release/gateway"
