#!/usr/bin/env bash
# fake-security.sh — stateful stand-in for /usr/bin/security in spawn tests (U1).
#
# lib/secrets.sh is the plugin's only toucher of the Keychain. Pointing it at
# this script through SPAWN_SECURITY_BIN (the SPAWN_CLAUDE_BIN precedent) lets
# the whole credential path run with no real Keychain, no unlock prompt, and no
# secret on this machine's login keychain.
#
# WHY IT RECORDS ARGV
# KTD10's whole claim is that a secret never appears in a process argument:
...
# (KTD10 again) and the assertions run across both invocations.
#
# FIDELITY THAT MATTERS (each behaviour below was verified against the real
# binary this session; a fixture that smoothed any of them over would make the
# defence it is testing vacuous):
