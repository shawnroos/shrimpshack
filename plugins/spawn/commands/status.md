...
Use the Skill tool to invoke: `spawn:status`

The skill owns the details: it runs `${CLAUDE_PLUGIN_ROOT}/lib/spawnctl.sh status` and interprets the one JSON object the script prints. A down gateway is a normal answer, not an error to retry — the script says so in the same JSON.
