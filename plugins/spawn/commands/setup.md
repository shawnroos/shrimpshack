...
Bare run (the whole path — prerequisites, both credentials, the install, the supervisor, harness wiring, a start, and the round-trip):
...
Three sub-verbs exist for narrow re-runs: `acquire` (fetch, build, promote), `supervisor [--install-dir DIR] [--consent-adopt-agent]` (adopt the launchd agent that already supervises the gateway), and `wire [--consent-shell-rc]` (wire the installed harnesses). Prefer the bare run — it is the only path that verifies.
...
| `8` | operator confirmation required — nothing was changed on this path | Ask the operator once, in plain words, using `error`. If they agree, re-invoke the same command adding the flag that matches `consent_required`: `shell-rc` → `--consent-shell-rc`, `adopt-agent` → `--consent-adopt-agent`. If they decline, stop and say what is not wired. Never pass a consent flag the operator did not agree to. |
