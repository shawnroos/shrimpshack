...
bind_session() {   # bind_session <team-id>
    . "$ROOT/lib/scope-record.sh"
    local n
    n="$(herdr_linear::session_propose "$1")"
    herdr_linear::session_confirm "$1" "$n"
}
...
# The organization case is gone, not skipped. The board's own binding could hold
# an organization scope and this proved it narrowed nothing; the session record
# that survived binds a team and has no way to express one, so there is no state
# left for the assertion to reach.

@test "a scope clause is never made from a session name or a scope that is not bound" {
    write_cfg "$GOOD"
    . "$ROOT/lib/scope-record.sh"
    herdr_linear::session_propose t-web >/dev/null
    run --separate-stderr herdr_linear::board_config_load
    printf '%s' "$output" | python3 -c '
import sys, json
