...
setup() {
    ...
    BASE="$WORK/wt/acme/frame-effects"
    PARENT_WT="$BASE/WEB-2670-tool-blur-backdrop"
    mkdir -p "$BASE"
    git -C "$PROJECT" worktree add -q -b feature/WEB-2670-tool-blur-backdrop "$PARENT_WT" >/dev/null 2>&1
    bind_as "$PARENT_WT" WEB-2670
    # R17. The parent's project has a space, and it is not the focused one.
    export FAKE_HERDR_WORKSPACES='wA=Plugins,wG=Frame Effects'
...
@test "children are made beside the parent, from the parent's repository, whatever the record says" {
    ...
    run herdr_linear::layout_build WEB-2670 WEB-3308 WEB-3307
    [ "$status" -eq 0 ]
    for c in WEB-3308 WEB-3307; do
        [ -d "$(col "$c")" ]
...
@test "a child whose issue cannot be read refuses the layout before anything is made" {
    ...
    run --separate-stderr herdr_linear::layout_build WEB-2670 WEB-3001 WEB-3002
    [ "$status" -eq 3 ]
    [[ "$stderr" == *"WEB-3002"* ]]
    [ "$(herdr_calls 'tab create')" = "0" ]
    [ ! -e "$(col WEB-3001)" ]
    [ -z "$(ls -A "$BASE" | grep -v '^WEB-2670-')" ]
}
...
@test "every column is split inside the layout's own tab" {
    ...
    for c in WEB-3001 WEB-3002; do
        pane="$(herdr_linear::journal_get WEB-2670 "pane.$c")"
...
@test "a parent that already has a tab in its space gets its columns there" {
    made="$(FAKE_HERDR_ALLOW_MUTATION=1 "$HERDR_BIN" tab create --workspace wG --label WEB-2670)"
...
@test "a retry whose journalled tab is gone makes a new tab in the space" {
    herdr_linear::journal_put WEB-2670 tab wG:t999
    herdr_linear::journal_put WEB-2670 tabpane wG:p0999
    run herdr_linear::layout_build WEB-2670 WEB-3001
    [ "$status" -eq 0 ]
    [ "$output" != "wG:t999" ]
    [ "$(herdr_calls 'tab create --workspace wG')" = "1" ]
    pane="$(herdr_linear::journal_get WEB-2670 pane.WEB-3001)"
...
@test "a retry whose journalled tab is in another space does not split there" {
    herdr_linear::journal_put WEB-2670 tab wA:t1
    run herdr_linear::layout_build WEB-2670 WEB-3001
    [ "$status" -eq 0 ]
...
@test "a child already started elsewhere is laid out in its own worktree" {
    ...
    run herdr_linear::layout_build WEB-2670 WEB-3001
    [ "$status" -eq 0 ]
    [ "$(herdr_linear::journal_get WEB-2670 worktree.WEB-3001)" = "$started" ]
...
@test "a tab herdr cannot be asked about is not replaced" {
    ...
    run herdr_linear::layout_build WEB-2670 WEB-3002
    [ "$status" -eq 3 ]
    [ "$(herdr_calls 'tab create')" = "1" ]
}
