...
for f in contain secrets sanitize record binding scope-record linear schemes reconcile description herdr-read states herdr-write repos start context context-filter create; do
  source "$R/lib/$f.sh"
done
