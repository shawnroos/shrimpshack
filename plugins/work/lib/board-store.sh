...
# The board ledger is per herdr session, because a ledger of pane ids means
# nothing outside the server that issued them. session_id is main's reading of
# the same socket path the board's own session helper used, and answers
# `default` for an unnamed session, so the path shape is unchanged.
command -v herdr_linear::session_id >/dev/null 2>&1 \
    || . "${BASH_SOURCE[0]%/*}/herdr-read.sh"

herdr_linear::_board_session_path() {
    local base="${1:-}" name sock line
    [ -n "$base" ] || return 1
    # session_id reads HERDR_SOCKET_PATH and refuses without it. The board runs
    # from places that do not export it -- a hook, a test harness -- where the
    # running herdr still answers, so fall back to asking it, which is what the
    # board's own session helper did before this used session_id.
...
EOF
        [ -n "${sock:-}" ] || return 1
        name="$(HERDR_SOCKET_PATH="$sock" herdr_linear::session_id)" || return 1
    else
        name="$(herdr_linear::session_id)" || return 1
    fi
    [ -n "$name" ] || return 1
...
herdr_linear::board_root() {
    local root
    root="$(herdr_linear::_board_session_path "$HERDR_LINEAR_STORE_DIR")" || return 1
    printf '%s/board' "$root"
}
