...
# start.sh refuses rather than self-healing these -- it asks its caller to have
# sourced them -- and this file is that caller for the board's sync path.
command -v herdr_linear::worktrees_root >/dev/null 2>&1 \
    || . "${BASH_SOURCE[0]%/*}/contain.sh"
command -v herdr_linear::open_session >/dev/null 2>&1 \
    || . "${BASH_SOURCE[0]%/*}/herdr-write.sh"
command -v herdr_linear::_state_type_of >/dev/null 2>&1 \
    || . "${BASH_SOURCE[0]%/*}/reconcile.sh"
