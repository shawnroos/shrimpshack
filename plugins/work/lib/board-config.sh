...
# The scope comes from the session record the plugin already keeps, which binds
# a session to a team and nothing else. The board's own binding offered team,
# project and initiative; only team survived, so a session bound to a project
# narrows nothing here rather than narrowing wrongly.
herdr_linear::_board_session_scope() {
    local team
    command -v herdr_linear::session_team >/dev/null 2>&1 \
        || . "${BASH_SOURCE[0]%/*}/scope-record.sh"
    team="$(herdr_linear::session_team 2>/dev/null)" || return 0
    [ -n "$team" ] || return 0
    printf 'team\t%s' "$team"
}
