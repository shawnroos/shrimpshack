...
# stricter rule than is_safe_identifier, which admits `.` and has no length cap
# and which every other caller depends on as it is.

# The charsets are enumerated for the reason is_safe_identifier gives: bash 3.2
# matches a range by locale collation. The first-character rule is what keeps
