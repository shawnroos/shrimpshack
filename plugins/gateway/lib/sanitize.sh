...
# gateway::sanitize_for_display <string>
...
gateway::sanitize_for_display() {
    local out rc
    if command -v jq >/dev/null 2>&1; then
        out="$(printf '%s' "$1" | jq -Rrs "$GATEWAY_SANITIZE_JQ_DEF strip_display_controls" 2>/dev/null)"
        rc=$?
        if [ $rc -eq 0 ]; then
            printf '%s' "$out"
            return 0
...
# gateway::sanitize_stream
# Filter form, for piping a file or a command's output to a terminal — the log
# tail in gatewayctl.sh and the seed-run stderr tail in launch.sh. Line-oriented
# (jq -R reads a line at a time), so it streams rather than buffering a whole
# log into memory.
gateway::sanitize_stream() {
    if command -v jq >/dev/null 2>&1; then
        jq -Rr "$GATEWAY_SANITIZE_JQ_DEF strip_display_controls" 2>/dev/null
    else
