...
1. Run the script:
   ...
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/lib/gatewayctl.sh" status
   ```

   Always through `${CLAUDE_PLUGIN_ROOT}/lib/gatewayctl.sh`, never by PATH lookup. (`gw` on your PATH is an unrelated binary; this plugin deliberately leaves it alone and both surfaces share the same pid and log files, so neither double-starts against the other.)
