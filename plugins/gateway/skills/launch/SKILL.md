...
1. Run the script with the seed prompt on stdin:
   ...
   Always through `${CLAUDE_PLUGIN_ROOT}/lib/launch.sh`, never by PATH lookup. (`gw` on your PATH is an unrelated binary; this plugin leaves it alone.)

2. Flags — this is the whole set:

   - `--alias <name>` — **required.** Must be served by the gateway; grammar `[A-Za-z0-9._-]+`, checked before any network call and before the alias is ever put into the attach command. `/gateway:status` lists what is served.
