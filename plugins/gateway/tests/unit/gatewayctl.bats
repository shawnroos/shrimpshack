...
@test "AE1 stale pidfile: gateway serving, pidfile names a dead pid -> up, no second start" {
    ...
    # GATEWAY_INSTALL_DIR is unset and the search root is EMPTY, so any attempt
...
@test "R3: the log is appended, never truncated, across restarts" {
    ...
    [ "$(grep -c 'gatewayctl start' "$WORK/.gateway.log")" -eq 2 ]
...
@test "install dir: an explicit env override is honoured and reported by status" {
    ...
    export GATEWAY_INSTALL_DIR="$WORK/custom-install"
...
@test "install dir: an override naming a missing directory fails hard, never falls through" {
    make_install "$GATEWAY_SEARCH_ROOT/gateway-9.9.9"
    export GATEWAY_INSTALL_DIR="$WORK/does-not-exist"
...
@test "install dir: an override with no gateway binary fails hard" {
    ...
    export GATEWAY_INSTALL_DIR="$WORK/empty-install"
...
@test "install dir: a DIRECTORY named gateway is not accepted as the binary" {
    ...
    export GATEWAY_INSTALL_DIR="$WORK/dirbin"
...
@test "install dir: with no override the NEWEST versioned dir wins, numerically" {
    ...
    make_install "$GATEWAY_SEARCH_ROOT/gateway-0.1.9"
    make_install "$GATEWAY_SEARCH_ROOT/gateway-0.1.10"
    make_install "$GATEWAY_SEARCH_ROOT/gateway-0.1.2"
    touch "$GATEWAY_SEARCH_ROOT/gateway-0.1.9/target/release/gateway"

    ctl status
    [ "$(echo "$output" | jq -r '.install_dir')" = "$GATEWAY_SEARCH_ROOT/gateway-0.1.10" ]
}
...
@test "install dir: no candidate at all is the unreachable class with a clear stderr message" {
    ...
    echo "$output" | grep -q 'no gateway install found'
    echo "$output" | grep -q "$GATEWAY_SEARCH_ROOT"
}
