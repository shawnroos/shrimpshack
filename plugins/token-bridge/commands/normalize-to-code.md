...
Code is the source of truth. Read the codebase's CSS design tokens and reconcile them into the connected Paper file: create new tokens, update changed values, and recreate any whose Paper type changed (scoped to the configured prefix). It never deletes: a live token absent from the source is reported under `prunable` and left in place, because a gap in the parse is indistinguishable from a deletion. Removing a token is a manual step in Paper — `prunable` is the list. Values Paper cannot represent (shadows, motion) are reported, never written.
