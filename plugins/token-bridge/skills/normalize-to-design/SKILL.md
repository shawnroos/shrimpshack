...
- A dark override block for the `-dark` twins, in the config's **primary** convention: `:root[data-theme="dark"] { … }` for a data-attribute convention, `@media (query) { :root { … } }` for a media-query one, or `:root.wcs-dark { … }` for a class one. A **file** convention emits TWO files — the base to `emitTarget` and the dark half to the convention's own `emitTarget` — and refuses if that second target is missing or collides with any input. The `-dark` suffix is stripped from both the property name and any `var(--…-dark)` alias referent.
