...
2. Read the exit code and JSON report:
   ...
   - **Exit 2 (bad args):** e.g. neither/both of `--file`/`--create-file`, a media-query convention with no `--query`, a class convention with no `--class`, or a file convention with no `--path`. Relay the `note`.
