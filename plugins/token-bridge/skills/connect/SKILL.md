...
- Optional: `--prefix` (custom-property namespace, e.g. `--brand-`; omit for all), `--emit-target` (Paper→CSS output; defaults to `<source>.generated.<ext>`), and the theme convention: `--convention data-attribute --attr data-theme --value dark` (default) or `--convention media-query --query "(prefers-color-scheme: dark)"`.
...
2. Read the exit code and JSON report:
   ...
   - **Exit 2 (bad args):** e.g. neither/both of `--file`/`--create-file`, or a media-query convention with no `--query`. Relay the `note`.
