---
name: lint-router
description: Run the right linters for the current repo, chosen by WHO the work is for, from a routes.json profile registry. Each profile matches by git origin or marker files and runs its linters — as an overlay on the repo's own config or standalone — on just the files you changed; a repo that matches no profile runs nothing. Use before committing/PR-ing to catch bugs and dated idioms in the code you changed. A SessionStart hook prepares routing automatically; invoke this to run the check. Manage routing with the discover-linters / add-linter / configure-linter / remove-linter / explain-routing skills — add eslint, ruff, clippy, shellcheck, prettier, or any tool. Triggers: finishing a code change, "run the lint check", "lint my changes", "run lint-router".
---
...
| --- | --- | --- |
| **work** | a repo you do work for — origin `*slateteams/*`, or an `@angular-eslint` `eslint.config.mjs` | a **curated** eslint correctness set **layered on the repo's own eslint** (overlay); risky auto-fixes off. In a team repo with no eslint of its own, nothing runs. |
| **personal** (default) | anything else — your own repos | a full standalone eslint config, self-contained via the bundled eslint. |
