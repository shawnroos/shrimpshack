...
- R28. When a launchd agent supervises the gateway, setup makes that agent's own starts authenticate without storing a credential anywhere new, and says what it changed. A supervised gateway that setup cannot reach is a gateway setup cannot claim to have configured.
...
- KTD21. **A supervisor is a third control surface; setup points it at a launcher rather than putting a secret in it.** (session-settled: user-directed — chosen over writing `GATEWAY_TOKEN` into the plist's `EnvironmentVariables`, which the user rejected because it puts a credential at rest where none was before; and over refusing to run on a supervised install, which fails on the one machine this was built for.) KTD4 assumed two surfaces sharing `~/.gateway.pid`: the plugin and the hand-written `gw`. A `launchd` agent is a third and outranks both — `KeepAlive` undoes a stop within seconds, `RunAtLoad` starts it at login, and its relaunch carries a bare environment that never sees the transient delivery file. Found live: the agent on the build machine has no `EnvironmentVariables` key, so once the config token was retired **every** respawn came up with an empty auth list, which that gateway serves as "no auth required".

  Setup therefore generates a launcher that reads `GATEWAY_TOKEN` from the Keychain at start and execs the gateway, and repoints the existing agent's `ProgramArguments` at it. The Keychain stays the only store — nothing new rests on disk. Putting that token in the gateway's exec-time environment is already sanctioned by KD3: it is loopback-only, worthless off `127.0.0.1:4000`, cheap to rotate, and already travels in every harness environment by design. The OpenRouter key is the one that must never reach an exec-time environment, and it does not — on a supervised install it continues to come from the install directory's own `.env.local`, which the gateway reads CWD-relative and which the agent's `WorkingDirectory` already points at.

  **The cost, stated rather than absorbed:** setup rewrites `ProgramArguments` in a plist the operator wrote, so the gateway starts through a file the plugin owns. That is a real escalation of what this plugin touches. It adopts an agent that already exists and never creates one; every other key in the plist survives byte-for-byte. Governs R28.
...
- **Approach:**
  ...
  6. Detect a `launchd` agent supervising the gateway (an agent whose `ProgramArguments` name the resolved binary) and repoint its `ProgramArguments` at a generated launcher that reads `GATEWAY_TOKEN` from the Keychain and execs the gateway, then reload it, so the agent's own starts authenticate (R28, KTD21). No credential is written to the plist. Record both the launcher and the plist in `changed`, and state that setup now owns a step in the startup path. Refuse to write a plist setup did not find already present — this step adopts an existing agent, it never creates one.
...
- **Test scenarios:**
  ...
  - A fixture agent plist is repointed at the generated launcher, and **no credential value appears anywhere in the written plist** — neither token nor key, asserted by grep over the whole file.
  - The generated launcher contains a Keychain read and no credential value, and execs the gateway binary the plist originally named.
...
- **On a supervised install, setup rewrites `ProgramArguments` in a plist the operator wrote** (KTD21), so the gateway starts through a launcher the plugin generates. No credential rests anywhere new — the launcher reads the Keychain at start — but the plugin now owns a step in a startup path it did not previously touch.
