...
- **Requirements:** R8.
- **Dependencies:** U1.
- **Files:** `plugins/work/lib/{contain,secrets,documents,herdr-write}.sh`, `plugins/work/docs/linear-conventions.md`, `plugins/work/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` (repo root — the `work` entry only), `plugins/work/tests/fixtures/descriptions/web-3184.md`, `plugins/work/tests/fixtures/fake-security.sh`, `plugins/work/tests/run-tests.sh`, and the seam across the ten test suites that export it. The four libraries, the conventions document and the two fixtures were added at dispatch; the plan's original list missed them. No `SKILL.md` or `commands/work.md` — U5 owns that prose.
