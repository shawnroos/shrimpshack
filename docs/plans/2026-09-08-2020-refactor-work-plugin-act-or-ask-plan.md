...
- KTD7. **R7's second signal derives from what `current_context` already reads, not a new mapping.** `lib/create.sh:37-52` resolves a project from the bound issue, then from the bound herdr workspace. The signal reports that project, `negative` when neither resolves, and `unknown` when herdr or Linear is unreachable. A declared repo-to-project mapping is out of scope. Governs R7.
