...
- Name resolution spawns four `python3` processes to read two fields from one already-fetched Linear response — `start_worktree_name` and `start_branch_name` each extract identifier and title independently, at `lib/start.sh:255-256` and `lib/herdr-write.sh:337-338`. It was four before this work and four after, so it is pre-existing rather than introduced, and the shared `_start_issue_field` helper now makes a single combined extractor cheap. Declined here because the path also makes a Linear API call, against which interpreter startup is noise, and the fix moves signatures at two call sites.
- The skills assume one shell across a document's fences, and the sync check assumes the same (see Implementation Constraints). Whether an agent actually keeps that shell between fences is unverified; if it does not, 34 fences exit 127 when run on their own. Settling which model the skills are written for, then making fences and check agree, is outside this plan's scope.
...
- `skill_lib_sync_check` joins every bash fence in a document into one string before collecting declarations (`run-tests.sh:441-442`), so a `source` in one fence satisfies a call in another. Deleting the conventions fence's own `source` from `skills/doc/SKILL.md`, whose other fences already load that lib, leaves `smoke` passing; the same deletion in `skills/describe/SKILL.md`, where no sibling fence loads it, is caught. This matches how the skills are written: 34 fences across all nine owned documents call plugin functions and load no library themselves, relying on an earlier fence's shell. Whether an agent keeps that shell between fences is unverified, and a fence run on its own exits 127. A new fence follows the document it sits in; a fence that must stand alone, as the conventions fences do, loads its own libraries.
