...
- R25's mechanism exists and was checked against `claude --help` on 2026-08-07. A launched session takes `--settings` (a file or inline JSON), `--permission-mode` (`manual`, `dontAsk`, `plan` and others), `--allowedTools` / `--disallowedTools`, and `--add-dir` for extra roots. Most importantly it takes `--setting-sources user,project,local`, which decides whether the child loads the operator's own settings at all — so a child can be launched strictly narrower than its launcher rather than inheriting whatever the operator has already allowed. Which combination expresses each ceiling is a planning choice.
