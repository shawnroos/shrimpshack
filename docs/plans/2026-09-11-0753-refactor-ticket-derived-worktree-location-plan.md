...
- AE1. **Covers R1, R3.** Given issue `WEB-3308` in an organisation whose URL key is `acme`, project `Frame Effects`, when a worktree is started for it from an unrelated directory, then the worktree is at `<worktrees-root>/acme/frame-effects/WEB-3308-export-panel-is-empty…` and no path segment came from the caller's directory.
...
- AE8. **Covers R4.** Given issue `WEB-3308`, when a worktree is started with the default prefix, then the branch is `feature/WEB-3308-…` and the directory is `WEB-3308-…`.
...
- AE11. **Covers R16.** Given a layout run from a worktree bound to parent `WEB-2670`, when children `WEB-3308` and `WEB-3307` are laid out, then both worktrees sit beside the parent's own worktree and are made from the parent's repository, whatever repository the recorded set holds.
...
- KTD2. **Both strings are built from `identifier + title-slug`, not from Linear's `branchName`.** Linear's `branchName` is lowercase (`web-3308-…`), so a directory derived from it cannot lead with an uppercase identifier. `herdr_linear::slug` preserves case, so an identifier survives slugging when the string is composed locally. Governs R3, R4.
...
- KTD22. **A layout refusal for the wrong directory is its own exit value, `LAYOUT_NOT_PARENT` (4).** Bad-name (2) says "a title cannot become a name", which is a different fix from "run this somewhere else". The check resolves the directory to its worktree top level, so a subdirectory of the parent's worktree counts as the parent's worktree, and it requires the binding to be `bound`, not only proposed. Governs R16.
- KTD23. **A layout run from a main checkout that is bound to the parent refuses.** Siblings of a main checkout sit beside the canonical repositories in the projects root, which is exactly where this plan keeps ephemeral trees out. The person runs `/work:start` on the parent first and lays out from that worktree. Governs R16, R13.
- KTD24. **Every child is fetched before the tab is made, and a journalled column keeps its journalled path.** One unreadable child refuses the whole layout with nothing made. A child whose worktree is already in the journal is not fetched again, so a title renamed between a failed run and its retry cannot derive a second worktree. Governs R16.
- KTD25. **Layout branches follow R4 as well.** A child's branch is `<prefix>/<IDENTIFIER>-<title-slug>`, built by the same `start_branch_name` as a started ticket, replacing the bare identifier slug the layout used before. Governs R4, R16.
- **A verification line in U5 cannot be met, and that is recorded rather than faked.** U5 says restoring `_make_worktree`'s argument-less repository read turns AE11 red. It cannot: the layout now refuses unless it runs from the parent's own worktree, so `worktree_repo "$PWD"` and `worktree_repo "<parent worktree>"` are the same repository by construction. The explicit parameter stays, because it removes the caller-directory read from the verb's contract, but no test can observe it. The mutation was run and stayed green.

- KTD26. **The one-known-answer rule is factored at its third use.** Teams, repositories and spaces each hand candidate lines to one helper that answers "the only one, or nothing". Each site keeps its own candidate reader, its own record and its own reason text. Governs R5–R7, R17–R19.
- KTD27. **A space's candidates are the bound workspace records for the issue's project that herdr still reports.** Read from the store (KTD13), filtered by one `workspace list`, so a closed space is not offered. When there is none, the reason reads the space the person is working from: unbound is the R18 proposal naming that space and the project; bound to another project is the R19 Misplaced report, worded like `check_placement`, offering both moves and choosing neither; no current space is a plain ask. A label that names the project changes nothing. Governs R17–R19.
- KTD28. **The tab a ticket owns is a field on that ticket's worktree binding, and is reused only when it is live and in the resolved space.** A recorded tab that herdr no longer has, or that sits in another space, is replaced by a new tab in the resolved space and the record is updated. A new tab is `tab create --workspace <space> --cwd <worktree> --label <IDENTIFIER> --no-focus`, and its root pane is the session. An existing tab gets `pane split <a pane in that tab> --direction right --cwd <worktree> --no-focus`. Nothing ever splits the focused pane. Governs R17, R20.
- KTD29. **When placement is a choice, `open_session` returns its own ask value and records a pending placement notice on the worktree's binding.** The notice is its own slot, not the pending-consent one, because a consent answer clears that slot. `ground.sh` surfaces it at the next session start, as it surfaces a deferred write. The verb cannot tell whether a person is watching, so it records on every path, the rule `consent_gate` already follows. The create tail stops discarding `open_session`'s stderr, which carried the question to nowhere. Governs R18, R21.
...
- KTD32. **The ask values are `SESSION_ASK` (6) for `open_session` and `LAYOUT_ASK` (5) for the layout.** 6 matches `START_ASK`, so a reader sees one number for "a question, nothing made" on the per-ticket verbs; the layout's enum already used 4. An issue with no project asks too: a space is a project, so there is nothing to look up. Governs R17, R19.
- KTD34. **A layout resolves the parent's space on every run, retries included, and holds a journalled tab to KTD28's rule.** This replaces an earlier version of KTD32 that let a retry skip the lookup and trust the journalled tab. A cross-model review showed why that was wrong: a tab closed between a failed run and its retry was split from on every retry, so the documented "re-run continues" recovery could never succeed, and a tab moved to another space took the remaining columns with it. The journal no longer records the tab's root pane; a live pane of the tab is read each run. Governs R17, R20.
- KTD35. **The existing-directory recovery asks git whether the path is a worktree.** `start_from_issue` adopts a directory at its derived path only when `git rev-parse --show-toplevel` names that same path. A directory that merely holds a `.git` entry is refused as `START_EXISTS`. Governs R15.
- KTD36. **Recording an answer drops candidates that are no longer on disk.** KTD18 re-asks when the only recorded repository has moved. Without this, the answer sat beside the dead path and every later start saw two candidates. A repository on an unmounted volume is dropped the same way, and is asked about again when it matters. Governs R6, R8.
- KTD37. **The project segment is composed like the title slug, and falls back to the team key.** `slug()` refuses a leading non-alphanumeric, and Linear project names start with emoji and brackets, so every start in such a project failed with nothing printed. A name with no letters or digits at all uses the lowercased team key; the record key stays the project id. Governs R1, R5a.
- KTD38. **A layout lays out a child in the child's own worktree when one already holds its branch.** R4 gives a started ticket and a layout column the same branch, so a child started with `/work:start` made every later layout of its parent fail on `worktree add -b`. The layout now asks git which worktree has the branch and uses it. A branch with no worktree still fails, as it did. Governs R4, R16.
- KTD39. **A failed read in placement says what failed, and a built layout clears its own placement question.** An unreadable issue or space list returns the failure value with a reason on stderr instead of an empty pane and silence; a space list that could not be read is never taken for an empty one. A layout that resolves its space clears the pending placement notice it may have recorded earlier. Governs R17, R21.
- KTD40. **Round-two corrections to KTD34 and KTD38.** A second review of the first fixes found three regressions, now closed. (1) "Herdr could not be asked about this tab" is not "the tab is gone": `tab_space` reads herdr's error code, and a read failure fails the layout rather than making a second tab. (2) A column journalled in a tab that has since closed is not done; it counts as done only while its pane is still in the current tab. (3) The worktree the layout reuses for a child must be a linked worktree that git does not report as prunable, and not bound to another issue. A child branch that exists only in the main checkout, or in no worktree, refuses the layout before anything is made. The existing-directory adoption also requires git's answer to be non-empty, so an unreadable directory is refused as existing.
- KTD41. **Round three: herdr's errors are on stderr, and "could not read" is never "absent".** herdr 0.9.0 writes the `tab_not_found` error object to stderr with an empty stdout, checked live. The fake had it on stdout, so `tab_space` passed every test while treating every closed tab as "herdr could not be asked" — a closed tab would have failed the layout and `open_session` on every run. The fake now answers on stderr and `tab_space` reads the error there. The pane readers returned nothing for both "no such pane" and "could not read"; they now tell them apart (KTD42). A worktree registered for the child's branch whose directory is gone, or whose binding record holds an unsafe identifier, is refused with a reason rather than adopted. A record that fails its mode check or does not parse still reads as absent, and that worktree is adopted; this is listed as an unapplied review finding.
- KTD42. **Round four: only the read that answered can say it failed.** Round three told "could not read" from "absent" by reading the snapshot a second time. A server busy for one read and readable on the next passed that check, so one failed read still made a second tab in `open_session` or a second pane in the layout. The pane readers now return 2 when their own snapshot could not be read (a non-zero exit, empty output, or no pane list) and 1 only for a snapshot that holds no match. The second read is gone. The fake gained `FAKE_HERDR_SNAPSHOT_FAILS_AT` (one failed read) and `FAKE_HERDR_SNAPSHOT_NO_PANES`.
- **Left open for Shawn: a same-issue worktree switched to another branch.** When this issue's own worktree is on a branch other than the one it was bound on, the binding reads as proposed and `/work:start` re-confirms it on the new branch. The plan kept the rebind branches unchanged (U4 step 8), and whether a retry should refuse instead is a product decision, not a mechanical fix. It is listed as an unapplied review finding on the PR.
...
- **Test scenarios:**
  - Covers AE8. `WEB-3308` with the default prefix yields branch `feature/WEB-3308-export-panel-is-empty…` and name `WEB-3308-export-panel-is-empty…`, and the branch is the name with the prefix in front.
