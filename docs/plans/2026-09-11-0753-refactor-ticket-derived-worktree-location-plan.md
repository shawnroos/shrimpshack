...
- KTD41. **Round three: herdr's errors are on stderr, and "could not read" is never "absent".** herdr 0.9.0 writes the `tab_not_found` error object to stderr with an empty stdout, checked live. The fake had it on stdout, so `tab_space` passed every test while treating every closed tab as "herdr could not be asked" — a closed tab would have failed the layout and `open_session` on every run. The fake now answers on stderr and `tab_space` reads the error there. The pane readers return nothing for both "no such pane" and "could not read"; a snapshot check now tells them apart, so a failed read never splits a second pane into a finished layout or makes a second tab. A worktree registered for the child's branch whose directory is gone, or whose binding record cannot be read, is refused with a reason rather than adopted.
